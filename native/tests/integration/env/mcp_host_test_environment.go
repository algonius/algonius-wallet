...
func (env *McpHostTestEnvironment) Setup(ctx context.Context) error {
	...
	// Create MCP client
	env.mcpClient = NewMcpHTTPClient(env.baseURL)

	// Initialize MCP client connection
	if err := env.mcpClient.Initialize(ctx); err != nil {
...
func (env *McpHostTestEnvironment) buildMcpHost() error {
	...
	// Build algonius-wallet-host from native/cmd/main.go, output to native/bin/algonius-wallet-host
	cmd := exec.Command("go", "build", "-mod=mod", "-o", "bin/algonius-wallet-host", "./cmd/main.go")
	cmd.Dir = "../../" // Run from native/tests/integration/env/, so ../../ is native/
	return cmd.Run()
}
...
func (env *McpHostTestEnvironment) waitForHostReady(ctx context.Context) error {
	...
	if env.baseURL == "" {
		log.Printf("[LOG-MONITOR] MCP host running in stdio mode, skipping HTTP readiness check")
		return nil
	}
...
func (env *McpHostTestEnvironment) GetMcpClient() *McpHTTPClient {
