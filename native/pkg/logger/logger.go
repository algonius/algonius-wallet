...
func getDefaultLogFilePath() string {
	// Default log directory: $HOME/.algonius-wallet/logs/native-host.log (hidden project folder)
	// If home directory is unavailable, fallback to system temp directory
	homeDir, err := os.UserHomeDir()
	if err == nil {
		return filepath.Join(homeDir, ".algonius-wallet", "logs", "native-host.log")
	}
	return filepath.Join(os.TempDir(), ".algonius-wallet", "logs", "native-host.log")
}
...
func buildZapLogger() (*zap.Logger, error) {
	...
	if logFile := os.Getenv("LOG_FILE"); logFile != "" {
		...
		logFilePath = filepath.Join(logDir, "native-host.log")
	} else {
